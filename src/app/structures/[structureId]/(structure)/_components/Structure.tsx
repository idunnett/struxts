...
import { EdgeData, NodeData, TBasicNode, TFloatingEdge } from "~/types"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import SearchCompaniesForm from "../../../_components/SearchCompaniesForm"
...
function Structure({ children }: Props) {
  ...
  const [addCompanyNodeOpen, setAddCompanyNodeOpen] = useState<{
    x: number
    y: number
  } | null>(null)
  const [createNewCompanyNodeOpen, setCreateNewCompanyNodeOpen] = useState<{
    x: number
    y: number
  } | null>(null)
  ...
  function handleAddNode(e: MouseEvent<HTMLDivElement>, isCompany?: boolean) {
    const { x, y } = reactFlowInstance?.screenToFlowPosition({
      x: e.clientX - 140,
      y: e.clientY - 30,
    }) ?? { x: 0, y: 0 }
    if (isCompany) {
      setAddCompanyNodeOpen({ x, y })
    } else {
      addNode({
        x,
        y,
  ...
  return (
    <>
      ...
      <Dialog
        open={!!addCompanyNodeOpen}
        onOpenChange={(open) => !open && setAddCompanyNodeOpen(null)}
      >
        <DialogContent onOpenAutoFocus={(e) => e.preventDefault()}>
          <DialogHeader>
            <DialogTitle>Available Companies to Add</DialogTitle>
            <DialogDescription hidden>
              Add a company to this structure
            </DialogDescription>
          </DialogHeader>
          <SearchCompaniesForm
            structureId={structure._id}
            nodePosition={addCompanyNodeOpen!}
            onCreateNewCompany={() => {
              setCreateNewCompanyNodeOpen(addCompanyNodeOpen)
              setAddCompanyNodeOpen(null)
            }}
            onAddCompany={() => setAddCompanyNodeOpen(null)}
          />
        </DialogContent>
      </Dialog>
      <Dialog
        open={!!createNewCompanyNodeOpen}
        onOpenChange={(open) => !open && setCreateNewCompanyNodeOpen(null)}
      >
        <DialogContent onOpenAutoFocus={(e) => e.preventDefault()}>
          <DialogHeader>
            <DialogTitle>Create a New Company</DialogTitle>
            <DialogDescription hidden>
              Create a new company to add to this structure
            </DialogDescription>
          </DialogHeader>
          <CreateNewCompany
            structureId={structure._id}
            nodePosition={createNewCompanyNodeOpen!}
            onCancel={() => setCreateNewCompanyNodeOpen(null)}
            onSave={() => setCreateNewCompanyNodeOpen(null)}
          />
        </DialogContent>
      </Dialog>
    </>

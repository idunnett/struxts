...
export default function BasicNode({
  ...
  const params = useParams() as { structureId: string }
    ...
    >
      ...
      >
        ...
        <Link
          href={
            data.isActive
              ? `/structures/${params.structureId}`
              : `/structures/${params.structureId}/nodes/${id}/info`
          }
        ...
        <div className="flex items-center justify-center gap-2">
          {data.company && <LucideBuilding2 className="h-4 w-4 shrink-0" />}
          <LineWrappingInput
            id={`line-wrapping-input-${id}`}
            value={data.company ? data.company.name : data.label}
            onChange={(e) =>
              data.onNodeDataChange?.(id, { label: e.target.value })
            }
            className={cn(
              "nodrag w-full grow break-words bg-transparent text-center text-sm outline-none",
              dragging && "cursor-grabbing",
            )}
            readOnly={!data.editable || !focused || !!data.company}
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={() => {
                    data?.onDelete?.(id)
                    router.replace(`/structures/${params.structureId}`)
                  }}
                >
                  Continue
                </AlertDialogAction>
